...
The solution to the problem would be to make a copy or use a smart pointers:
```cpp
// solution with smaart pointers!!
#include <string>
#include <iostream>
#include <memory>
...
Person makeDefaultPerson(){
    const auto dummy_name = std::make_shared<std::string>("Bob"); // you have to initialise using a smart_pointer
                // otherwise, you are doommed to make an unnecessary copy (or you will have to use weird `move` semantics)
    int dummy_age = 23;
    return Person{
        .name = dummy_name, // the shared pointer of Person object will increase the reference counter of string "Bob"
        .age = dummy_age
    };
